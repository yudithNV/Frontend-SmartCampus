...
// ─────────────────────────────────────────────────────────────────────────────
// CORRECCIÓN CLAVE: apiRequest ahora acepta (endpoint, options)
// La firma vieja era (endpoint, method, data) — eso causaba [object Object]
// ─────────────────────────────────────────────────────────────────────────────
async function apiRequest(endpoint, options = {}) {
  try {
    ...
    // El browser lo calcula solo con el boundary correcto
...
// ─────────────────────────────────────────────────────────────────────────────
// Servicios de Carreras / Categorías / Ubicaciones
// ─────────────────────────────────────────────────────────────────────────────
...
export const complaintService = {
  ...
  /**
   * Paso 1: Crear reclamo sin archivos — POST /api/complaints
   * Body JSON: { title, category, body }
   * ⚠️  El campo es "body", NO "description"
   */
  create: async (dto) => {
  ...
  /**
   * Paso 2: Subir adjuntos — POST /api/complaints/{id}/adjuntos
   * Multipart/form-data con campo "files" (array)
   * Se llama DESPUÉS de create() usando el id del reclamo creado
   */
  addAttachments: async (id, files) => {
    const formData = new FormData()
    files.forEach(file => formData.append('files', file))

    // isFormData=true → apiRequest NO pondrá Content-Type
